  ...
  return (
    <div className="pt-24 pb-0 bg-[#f8f9fa] min-h-screen font-sans text-gray-900">
      <div className="max-w-[95rem] mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* Breadcrumb */}
        <div className="flex items-center gap-2 text-[13px] font-semibold text-gray-500 mb-8 mt-4">
        ...
        <div className="flex flex-col lg:flex-row gap-12 mb-16">
          ...
          <div className="w-full lg:w-1/2 flex gap-4 h-[500px]">
            {/* Thumbnails */}
            <div className="flex flex-col gap-3 w-20 shrink-0 overflow-y-auto hide-scrollbar">
              {product.images.map((img, idx) => (
                <button 
                  key={idx}
                  onClick={() => setActiveImage(idx)}
                  className={`w-full aspect-square rounded-lg border-2 overflow-hidden transition-all ${activeImage === idx ? 'border-blue-600' : 'border-gray-200 hover:border-gray-400'}`}
                >
            ...
            {/* Main Image */}
            <div 
              className="flex-grow rounded-2xl overflow-hidden bg-white border border-gray-200 relative cursor-zoom-in"
            ...
            >
              <img 
                ...
                alt={product.title} 
                className={`w-full h-full object-fill transition-transform duration-200 ease-out`}
                style={{ 
          ...
          <div className="w-full lg:w-1/2 flex flex-col">
            ...
            <ul className="space-y-3 mb-8">
              {product.features.map((feature, idx) => (
                <li key={idx} className="flex items-start gap-3">
                  ...
                  <span className="text-[14px] font-semibold text-gray-800">{feature}</span>
                </li>
              ))}
            </ul>
            ...
            <div className="flex items-center gap-4 mb-10">
              ...
              <a href="#" className="bg-white border-2 border-green-500 text-green-600 px-8 py-3.5 rounded-lg font-bold text-[13px] hover:bg-green-50 transition-colors flex items-center gap-2 shadow-sm">
                WHATSAPP US
              </a>
            </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-auto">
              {product.infoBoxes.map((box, idx) => (
                <div key={idx} className="bg-white border border-gray-200 rounded-xl p-4 flex flex-col justify-center gap-2 shadow-sm">
                  <div className="text-gray-400">{IconMap[box.icon] || <Settings className="w-5 h-5" />}</div>
                  <div>
                    <h5 className="text-[11px] font-bold text-gray-500 uppercase tracking-wide mb-0.5">{box.title}</h5>
                    <p className="text-[12.5px] font-bold text-gray-900 leading-snug">{box.value}</p>
                  </div>
                </div>
              ))}
            </div>
              ...
              <div className="flex flex-col lg:flex-row gap-12">
                <div className="w-full lg:w-1/2">
                  ...
                  <div className="space-y-4 mb-8">
                    {product.longDesc ? (
                      <p className="text-[15px] font-medium text-gray-700 leading-relaxed whitespace-pre-line">
                        {product.longDesc}
                      </p>
                    ) : (
