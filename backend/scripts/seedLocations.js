...
const seedLocations = async () => {
  try {
    ...
    // Insert all in bulk (ignore duplicates based on slug if they exist by catching the error)
    // Actually, distinct names in the list so should be fine.
    // Some districts might share names (like Aurangabad in Bihar & Maharashtra). We should ensure unique slugs.
    
    // De-duplicate logic for slugs
    const slugCounts = {};
    const uniqueLocations = locationDocs.map(loc => {
      let finalSlug = loc.slug;
      if (slugCounts[loc.slug]) {
        slugCounts[loc.slug]++;
        finalSlug = `${loc.slug}-${slugCounts[loc.slug]}`;
      } else {
        slugCounts[loc.slug] = 1;
      }
      return { ...loc, slug: finalSlug };
    });

    await Location.insertMany(uniqueLocations);
    console.log(`Successfully seeded ${uniqueLocations.length} locations!`);
